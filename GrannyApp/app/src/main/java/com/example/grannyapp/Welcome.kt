...
class Welcome : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Welcome.setText(UserSignedIn.getInstance().arrayUsers[Signedin].Name)
        var image: Bitmap? = null
        val imOutput :ImageView = findViewById(R.id.imWelcome)
        val handler = Handler(Looper.getMainLooper())
        val executor = Executors.newSingleThreadExecutor()
        executor.execute{
            val imageURL = UserSignedIn.getInstance().arrayUsers[Signedin].imageURL
            try
            {
        ...
        executor2.execute {
            ...
            Handler(Looper.getMainLooper()).post{
                userAdapter.submitList(UserSignedIn.getInstance().arrayUsers)
            }
        }
    }
}
